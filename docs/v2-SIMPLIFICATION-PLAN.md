...
## Progress log
- **Wave 2 — DONE & merged, gated (consumer-resolution), 699 tests.** `/org`+`/advanced` subpath entry points (`exports` map + multi-entry tsup); org/advanced/internal moved to subpaths with FULL root back-compat (`@deprecated` root re-exports; 135 value + 232 type exports preserved); `NamingMode` enum → flat `omitPrefix`/`omitLocation`/`preview` booleans.
- **Wave 3 — DONE & merged @ `6422996`, zero-replace gate PASSED (all 3 stacks), 745 tests.** Opt-in deterministic auto-label (default OFF → byte-identical); single+bulk merged into one `string|string[]` class with URN-preserving `CloudInfraBulkX` `@deprecated` aliases. **Review teams (4 reviewers × 6 areas) found 1 Critical + 6 Major + many Minor — ALL fixed in one consolidated pass:** W3-C `serviceAccount` nullability (getter); `AccessMatrixCases` back-compat (wide restored, `AccessMatrixCasesStrict` = narrow); `/org` lazy billing default; `expandPrincipals` gated on real bulk-ness (single account → wrapper path, byte-identical, no `__name` dependency); `splitMetaArgs` extended to `string[]` (single source of truth, incl. `CloudInfraWIPProvider`); `EXTRA_TYPE_ALIASES` collision guard; `NAMING_ARG_KEYS` coupling; `isolatedModules`; + tests. Gate confirmed: counts identical to baseline, zero replace, zero IAM-binding rename (single-account target/principal paths byte-identical).
...
- **Wave 4 — DONE & merged @ `8ebdae4`, 762 tests, review-team-cleared.** `CloudInfraService` coordinator (plain class, NOT a ComponentResource → factory output byte-identical to direct construction): declare `{domain,location}` once; 11 factories (bucket/account/cloudRun/cloudRunJob/secret/backendService/alb/instance/certificateMap/repository/database) inheriting shared naming (per-call overrides), auto-registered into an internal output manager exposed via `outputs()`; opt-out via `{register:false}`. NO IAM surface (firm rule). Additive — no preview gate (no consumer adopts it yet). Review Major (test/doc mismatch) resolved: dead capture scaffolding removed, docstring trimmed to match the robust generated-name+URN+flat-key proof.

## PROGRAM COMPLETE (2026-06-28)
All four waves merged + gated + review-team-cleared. v2 @ `8ebdae4`, 762 green, pushed. Next: SHIP path (preview packages + admin-app canary) — user decision.
